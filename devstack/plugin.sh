...
function create_nimble_accounts {
    ...
    if [[ "$KEYSTONE_CATALOG_BACKEND" = 'sql' ]]; then
        get_or_create_service "nimble" "baremetal_compute" "Baremetal Compute v1"
...
function configure_nimble {
    ...
    iniset ${NIMBLE_CONF_FILE} ironic admin_url "http://${KEYSTONE_AUTH_HOST}:5000/v2.0"
    iniset ${NIMBLE_CONF_FILE} ironic api_endpoint "http://${$SERVICE_HOST}:6385"
...
function init_nimble {
    ...
    python ${NIMBLE_DIR}/nimble/cmd/dbsync.py --config-file ${NIMBLE_CONF_FILE}  upgrade
}
