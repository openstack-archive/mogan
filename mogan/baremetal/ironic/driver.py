...
class IronicDriver(base_driver.BaseEngineDriver):
    ...
    def _port_resource(self, port):
        ...
        dic = {
            ...
            'internal_info': str(port.internal_info),
            'extra': dict(port.extra),
        }
        return dic

    def _port_group_resource(self, portgroup):
        """Helper method to create resource dict from portgroup stats."""
        dic = {
            'address': str(portgroup.address),
            'node_uuid': str(portgroup.node_uuid),
            'port_uuid': str(portgroup.uuid),
            'internal_info': str(portgroup.internal_info),
            'extra': dict(portgroup.extra),
        }
        return dic
    ...
    def _get_manageable_nodes(self):
        ...
        for node in node_list:
            if str(node.resource_class) is None:
                continue
            ...
            node.portgroups = [self._port_group_resource(portgroup)
