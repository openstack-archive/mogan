...
class BaremetalComputeClient(rest_client.RestClient):
    ...
    def server_associate_floatingip(self, server_id, floatingip, fixed_ip):
        uri = '%s/servers/%s/networks/floatingips' % (
            self.uri_prefix, server_id)
        body = {"address": floatingip, 'fixed_address': fixed_ip}
        body = self.serialize(body)
        resp, body = self.post(uri, body)
        self.expected_success(204, resp.status)
        if body:
...
class Manager(manager.Manager):
    ...
    def set_compute_floatingips_client(self):
        self.network_flaotiingip_client = fip_cli.FloatingIPsClient(
            self.auth_provider,
            **self.network_params)
