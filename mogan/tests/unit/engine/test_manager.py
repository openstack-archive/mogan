...
class ManageServerTestCase(mgr_utils.ServiceSetUpMixin,
    ...
    def test_manage_servers(self, put_allocations_mock, show_port_mock,
                            adopt_mock, get_state_mock, rollback_quota_mock):
        ...
        server = obj_utils.create_test_server(
            self.context, status=states.BUILDING, node_uuid=None)

        adopt_mock.side_effect = None
        get_state_mock.return_value = 'power on'
        self.service.manage_server(self.context, server, node)
    ...
    @mock.patch.object(network_api.API, 'show_port')
    @mock.patch.object(report_api, 'put_allocations')
    def test_manage_servers_with_exception(self, put_allocations_mock,
                                           show_port_mock,
                                           adopt_mock,
                                           get_state_mock,
                                           rollback_quota_mock):
