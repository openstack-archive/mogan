...
class SchedulerReportClient(object):
    ...
    def _put_provider_aggregates(self, rp_uuid, aggs):
        ...
        if resp.status_code == 200:
            existing_aggs = list(self._provider_aggregate_map[rp_uuid])
            self._provider_aggregate_map[rp_uuid] = \
                list(set(existing_aggs + aggs))
            data = resp.json()
            return set(data['aggregates'])
    ...
    def add_aggregate_node(self, aggregate_uuid, node):
        rps = self._resource_providers
        ...
        self._put_provider_aggregates(rp_uuid, [aggregate_uuid])

    def remove_aggregate_node(self, aggregate_uuid, node):
        pass
