    ...
    def _get_filtered_affzs_nodes(self, context, server_group, filtered_nodes,
        ...
        def _log_and_raise_error(policy):
            LOG.error("No enough nodes filtered, request %(num_svr)s "
                      ...
                      "policy specified. It maybe because the filtered nodes "
                      "cannot match server group policy request, The filtered "
                      "nodes before matching the server group policy are: "
                      "%(filtered_nodes)s.",
