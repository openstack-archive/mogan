...
class FilterScheduler(driver.Scheduler):
    ...
    def _get_filtered_affzs_nodes(self, context, server_group, filtered_nodes,
        ...
        def _log_and_raise_error(policy):
            LOG.error("No enough nodes filtered, request %(num_svr)s servers "
                      "with server group %(group)s with %(policy)s policy "
                      "specified. The filtered nodes are: %(filtered_nodes)s.",
                      {"num_svr": num_servers, "group": server_group.name,
                       "policy": policy, "filtered_nodes": filtered_nodes})
            msg = (_("No enough nodes filtered, request %(num_svr)s servers "
