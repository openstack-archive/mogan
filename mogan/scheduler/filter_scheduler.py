    ...
    def _get_filtered_affzs_nodes(self, context, server_group, filtered_nodes,
        ...
        if 'affinity' in server_group.policies:
            ...
            if selected_affz:
                ...
                if num_affz_nodes < num_servers:
                    msg = _("No enough nodes filtered, request %(num_svr)s "
                            "servers, only %(num_nodes)s available in "
            ...
            if len(selected) < num_servers:
                msg = ('Not enough affinity zones with enough available nodes '
                       'found for servers request with anti-affinity '
                       'requirement, request %(num_svr)s servers in different '
                       'affinity zones, but only  %(num_affzs)s available '
                       'affinity zones'
                       % {'num_svr': num_servers, 'num_affzs': len(selected)})
                raise exception.NoValidNode(_(msg))
    ...
    def schedule(self, context, request_spec, filter_properties=None):
        ...
        def _schedule(self, context, request_spec, filter_properties):
            ...
            if not server_group:
                ...
                self._consume_nodes_with_server_group(
