...
from mogan.api.controllers.v1.schemas import servers as inst_schemas
...
class ServerStatesController(ServerControllerBase):
    ...
    def lock(self, server_uuid, target):
        ...
        # Target is True, means lock an server
        if target:
            pecan.request.engine_api.lock(context, rpc_server)

        # Else, unlock the server
        else:
            # Try to unlock an server with non-admin or non-owner
    ...
    def provision(self, server_uuid, target):
        ...
        :param server_uuid: UUID of an server.
...
class FloatingIPController(ServerControllerBase):
    ...
    def delete(self, server_uuid, address):
        """Dissociate floating_ip from an server.
...
class ServerNetworks(base.APIBase):
    """API representation of the networks of an server."""
...
class ServerCollection(base.APIBase):
    ...
    def convert_with_links(servers_data, fields=None):
        ...
        collection.servers = [Server.convert_with_links(inst, fields)
                                for inst in servers_data]
        return collection


class ServerConsole(base.APIBase):
    """API representation of the console of an server."""
...
class ServerController(ServerControllerBase):
    ...
    def post(self, server):
        ...
        """
        validation.check_schema(server, inst_schemas.create_server)
