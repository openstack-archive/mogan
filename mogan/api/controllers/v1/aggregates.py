...
class AggregateNodeController(rest.RestController):
    ...
    def _check_aggregates_conflict(node, node_aggregates, key):
        ...
        conflicts = [
            ...
            agg.metadata[key] != db_aggregate.metadata[key]]
        if conflicts:
    ...
    def post(self, aggregate_uuid, node):
        ...
        if 'availability_zone' in db_aggregate['metadata']:
            self._check_aggregates_confclit(
...
class AggregateController(rest.RestController):
    ...
    def _check_metadata_conflicts(aggregate_uuid, key, value, nodes):
        ...
        conflict_azs = [agg.metadata[key] for agg in filtered_aggs
                        if agg.metadata[key] != value and
                        agg.uuid != aggregate_uuid]
        if conflicts:
            msg = _("One or more nodes already in different "
                    "%(key)s(s) %(conflict)s") % {"key": key,
                                                  "conflicts": conflicts}
            raise wsme.exc.ClientSideError(
                msg, status_code=http_client.BAD_REQUEST)

    def _is_safe_to_update_metadata(patch, aggregate_uuid):
        ...
        for patch_dict in patch:
            for key in keys:
                if patch_dict['path'] == '/metadata/' + key \
                    ...
                    if not patch_val[key]:
                        msg = _("Aggregate %(name)s does not support empty "
                                "named %(key)s") % {"name": aggregate.name,
                                                    "key": key}
                        raise wsme.exc.ClientSideError(
                            msg, status_code=http_client.BAD_REQUEST)
                    else:
                        self._check_metadata_conflicts(
                            aggregate_uuid, key, patch_val[key],
                            nodes['nodes'])
