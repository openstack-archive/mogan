...
class Server(base.MoganObject, object_base.VersionedObjectDictCompat):
    ...
    @staticmethod
    def _from_db_object(server, db_inst, expected_attrs=None):
        """Method to help with migration to objects.

        Converts a database entity to a formal object.

        :param server: An object of the Server class.
        :param db_inst: A DB Server model of the object
        :return: The object of the class with the database entity added
        """
        for field in set(server.fields) - set(OPTIONAL_ATTRS):
            server[field] = db_inst[field]
    ...
    def _from_db_object_list(db_objects, cls, context):
        ...
        for obj in db_objects:
            ...
            servers.append(Server._from_db_object(cls(context),
                                                      obj,
                                                      expected_attrs))
        return servers
    ...
    def get(cls, context, uuid):
        ...
        if db_server["status"] == "error":
            expected_attrs.append("fault")
        server = Server._from_db_object(cls(context),
                                            db_server,
                                            expected_attrs)
        return server
