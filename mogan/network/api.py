...
class API(object):
    ...
    def _get_interface_by_instance(self, client, instance_uuid):
        interfaces = self._safe_get_interfaces(client, instace = instance_uuid)
        if len(interfaces) == 0:
            raise exception.InterfaceNotFoundForInstance(
                    instance = instance_uuid)
        elif len(interfaces) > 1:
            raise exception.InterfaceMultipleFoundForInstance(
                    instance = instance_uuid)
        return interfaces[0]
    ...
    def _unbind_ports(self, context, client, interface):
        ...
        except client_exc.PortNotFoundClient:
