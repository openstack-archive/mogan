...
class Connection(api.Connection):
    ...
    def _server_group_members_add(self, context, group_id, members,
        ...
        query = model_query(context, models.ServerGroupMember).filter_by(
            group_id=group_id)
        if delete:
            query.filter(~models.ServerGroupMember.server_id.in_(
                all_members)).delete(synchronize_session=False)
        query = query.filter(models.ServerGroupMember.server_id.in_(
            all_members))
        already_existing = set()
        for member_ref in query.all():
            already_existing.add(member_ref.server_id)
        with _session_for_write() as session:
            for server_id in members:
                if server_id in already_existing:
                    continue
                member_ref = models.ServerGroupMember()
                member_ref.update({'server_id': server_id,
                                   'group_id': group_id})
                session.add(member_ref)
        return members

    def _server_group_id(self, context, group_uuid):
    ...
    def server_group_create(self, context, values, policies=None,
        ...
        with _session_for_write() as session:
            try:
                session.add(server_group_ref)
                session.flush()
            except db_exc.DBDuplicateEntry:
                raise exception.ServerGroupExists(key_name=values['name'])
        if policies:
            self._server_group_policies_add(context, server_group_ref.id,
                                            policies)
        else:
            server_group_ref._policies = []
        if members:
            self._server_group_members_add(context, server_group_ref.id,
                                           members)
        else:
            server_group_ref._members = []
    ...
    def server_group_delete(self, context, group_uuid):
        ...
        for c in columns_to_join:
            query = query.options(orm.joinedload(c))
        query = query.filter(group_uuid == group_uuid)
        query.delete()

    def server_group_get_all(self, context):
        """Get all groups."""
        columns_to_join = ['_policies', '_members']
        query = model_query(context, models.ServerGroup)
        for c in columns_to_join:
