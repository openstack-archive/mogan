...
In order to use this feature, the service ``mogan-serialproxy`` needs to run.
...
"""),
    ...
    cfg.URIOpt('base_url',
               ...
               help="""
The URL an end user would use to connect to the ``mogan-serialproxy`` service.

The ``mogan-serialproxy`` service is called with this token enriched URL
and establishes the connection to the proper instance.

Related options:

* The IP address must be identical to the address to which the
  ``mogan-serialproxy`` service is listening (see option ``serialproxy_host``
  in this section).
...
"""),
    cfg.StrOpt('proxyclient_address',
               default='127.0.0.1',
               help="""
The IP address to which proxy clients (like ``mogan-serialproxy``) should
connect to get the serial console of an instance.

This is typically the IP address of the host of a ``mogan-compute`` service.
"""),
]
...
CLI_OPTS = [
    cfg.StrOpt('serialproxy_host',
               default='0.0.0.0',
               help="""
The IP address which is used by the ``mogan-serialproxy`` service to listen
for incoming requests.

The ``mogan-serialproxy`` service listens on this IP address for incoming
...
"""),
    cfg.PortOpt('serialproxy_port',
                default=8866,
                help="""
The port number which is used by the ``mogan-serialproxy`` service to listen
for incoming requests.

The ``mogan-serialproxy`` service listens on this port number for incoming
connection requests to instances which expose serial console.
