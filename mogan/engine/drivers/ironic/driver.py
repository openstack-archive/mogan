...
from mogan.engine.drivers import base_driver
from mogan.engine.drivers.ironic import ironic_states

LOG = logging.getLogger(__name__)


class IronicEngineDriver(base_driver.BaseEngineDriver):

    _NODE_FIELDS = (
        'uuid', 'power_state', 'target_power_state', 'provision_state',
    ...
    def destroy_node(self, node_uuid):
    ...
    def set_power_state(self, instance, state):
        self.ironicclient.call("node.set_power_state", instance.uuid, state)
        timer = loopingcall.FixedIntervalLoopingCall(
            self._wait_for_power_state, instance)
        timer.start(interval=CONF.engine.check_retry_interval).wait()

    def is_node_unprovision(self, node):
        if node.provision_state in \
                (ironic_states.ACTIVE, ironic_states.DEPLOYFAIL,
