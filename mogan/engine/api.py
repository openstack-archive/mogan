...
class API(object):
    ...
    def _check_requested_image(self, context, image, partitions):
        ...
        if image['status'] != 'active':
            raise exception.ImageNotActive(image_id=image.uuid)
    ...
    def _create_server(self, context, flavor, image_uuid,
        ...
        if self._is_whole_disk_image(context, image) and partitions:
            raise exception.PartitionsNotSupport(image_id=image.uuid)
