...
class API(object):
    ...
    def _set_default_partitions(self, context, image):
        """Set default partitions with image's min_disk or size"""
        # Image min_disk is in gb, size is in bytes.
        image_min_disk = int(image.get('min_disk') or 0) * units.Gi
        image_size = (int(image.get('size') or 0) / units.Gi) + 1

        root_gb = max(image_min_disk, image_size)
        return {'root_gb': root_gb}
    ...
    def _create_server(self, context, flavor, image_uuid,
        ...
        if (not iwdi) and (not partitions):
            # the deploy will fail if we don't set root partition size
            # for parition images.
            partitions = self._set_default_partitions(context, image)
