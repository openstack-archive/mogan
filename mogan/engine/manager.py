...
class EngineManager(base_manager.BaseEngineManager):
    ...
    def create_instance(self, context, instances, requested_networks,
                        request_spec=None, filter_properties=None):
        """Perform a deployment."""
        LOG.debug("Starting instance...", instance=instances)
        # TODO(little): send a notify for each instance?
        ...
        def _do_create_instance(instance):
            try:
            ...
            def _run_flow():
                # This code executes create instance flow. If something goes
                # wrong, flow reverts all job that was done and reraises an
                # exception. Otherwise, all data that was generated by flow
                # becomes available in flow engine's storage.
            ...
            try:
                ...
                # InvalidState, if this event is not allowed in the current
                # state.
                fsm.process_event('done')
                instance.power_state = ironic.get_power_state(
                    self.ironicclient, instance.uuid)
                instance.status = fsm.current_state
                instance.launched_at = timeutils.utcnow()
                instance.save()
                LOG.info(_LI("Created instance %s successfully."),
                         instance.uuid)
            finally:
                return instances

        for instance in instances:
            utils.spawn_n(_do_create_instance, instance)
