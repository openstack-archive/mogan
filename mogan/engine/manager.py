    ...
    def manage_server(self, context, server, node_uuid):
        ...
        try:
            ...
            self.scheduler_client.reportclient.put_allocations(
                node['uuid'], server.uuid, {}, server.project_id,
                server.user_id)
        ...
        try:
            ...
            availability_zone = node.get('availability_zone', None)
            if availability_zone:
                # check availability zone
                azs = self.engine_api.list_availability_zones(context)
            ...
            with excutils.save_and_reraise_exception():
                self._rollback_servers_quota(context, -1)
                self.driver.unadopt(node, server)
