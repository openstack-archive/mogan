...
class EngineManager(base_manager.BaseEngineManager):
    ...
    def manage_server(self, context, server, node_uuid):
        def _manage_server(self, context, server, node):
            ...
            try:
                self.driver.manage(server, node)
            except Exception as e:
                raise exception.ManageServerFailed(reason=str(e))
        ...
        try:
            _manage_server(self, context, server, node)
        except exception.ManageServerFailed:
            with excutils.save_and_reraise_exception():
                self._rollback_servers_quota(context, -1)
                self.driver.unmanage(server, node)
        except Exception:
            with excutils.save_and_reraise_exception():
                self._rollback_servers_quota(context, -1)
