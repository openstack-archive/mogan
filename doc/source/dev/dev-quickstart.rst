...
Create devstack/local.conf with minimal settings required to enable Nimble::
    ...
    # NOTE(RuiChen): It's ok to enable Horizon, Nova and Cinder services,
    #                they don't impact Nimble at all, disable them in the demo
    #                configuration to only deploy the dependent services.
    # Disable Horizon
