...
the env user credentials, you can also manually specify the user credentials.
Use `python adoption.py --help` to get more details.
"""
...
def _construct_nics(ctxt, client, bm):
    ...
    nics_object = objects.ServerNics(ctxt)
    for port in ports:
        nic_dict = {'port_id': port['id'],
                    ...
                    'server_uuid': bm.id}
        nics_object.objects.append(objects.ServerNic(
            ctxt, **nic_dict))
    return nics_object
...
def _add_flavor(ctxt, flavor_uuid):
    try:
        objects.Flavor.get(ctxt, flavor_uuid)
    except exception.FlavorNotFound:
        inst_type_obj = objects.Flavor(
            ctxt, uuid=flavor_uuid, name='nova_adoption_flavor',
            is_public=True, description='flavor created for bms '
                                        'adopted from Nova')
...
def handle_bms(ctxt, client, bms, nodes):
    for bm, node in zip(bms, nodes):
        ...
        if task_state is not None:
            print("The server %s in task state: %s, skipped to adopt." % (
                bm.id, task_state))
            continue
        ...
        inst_object = objects.Server(ctxt, **mogan_bm_attrs)
