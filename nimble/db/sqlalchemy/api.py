...
def _dict_with_extra_specs(inst_type_query):
    """Takes an instance type query returned by sqlalchemy and returns
    it as a dictionary, converting the extra_specs entry from a list of dicts:
...
class Connection(api.Connection):
    ...
    def instance_type_destroy(self, instance_type_uuid):
        with _session_for_write():
            ...
            extra_query = model_query(models.InstanceTypeExtraSpecs)
            extra_query = add_identity_filter(extra_query, type_id)
            extra_query.delete()
...
def _type_get_id_from_type(type_id):
    result = _type_get_id_from_type_query(type_id).first()
    if not result:
