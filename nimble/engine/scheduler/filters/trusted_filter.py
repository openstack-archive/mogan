...
class AttestationService(object):
    ...
    def __init__(self):
        self.api_url = CONF.trusted_node.attestation_api_url
        self.host = CONF.trusted_node.attestation_server
        self.port = CONF.trusted_node.attestation_port
        self.auth_blob = CONF.trusted_node.attestation_auth_blob
        self.key_file = None
        self.cert_file = None
        self.ca_file = CONF.trusted_node.attestation_server_ca_file
        self.request_count = 100
        # If the CA file is not provided, let's check the cert if verification
        # asked
        self.verify = (not CONF.trusted_node.attestation_insecure_ssl
...
class ComputeAttestationCache(object):
    ...
    def __init__(self):
        ...
        computes = objects.instance.Instance.list(admin)
    ...
    def _cache_valid(self, host):
        ...
        if host in self.compute_nodes:
            ...
            if not timeutils.is_older_than(
                             node_stats['vtime'],
                             CONF.trusted_node.attestation_auth_timeout):
                cachevalid = True
        return cachevalid
...
class TrustedFilter(filters.BaseNodeFilter):
    ...
    def node_passes(self, node_state, spec_obj):
        ...
        if trust:
            return self.compute_attestation.is_trusted(node, trust)
        return True
