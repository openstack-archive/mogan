...
class ScheduleNodeTask(task.Task):
    ...
    def execute(self, context, instance, request_spec):
        filter_properties = {}
        try:
            top_node = self.engine_manager.scheduler.schedule(context,
                request_spec,
...
def get_flow(context, instance, requested_networks, request_spec):
    create_what = {
        'context': context,
        'instance': instance,
    ...
    flow = linear_flow.Flow('create_instance')
    ...
    return taskflow.engines.load(flow, store=create_what)
