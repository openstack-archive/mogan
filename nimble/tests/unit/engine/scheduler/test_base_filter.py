...
'''
    def test_get_filtered_objects_info_and_debug_log_none_returned(self):
        ...
        fake_nodes = [node_manager.NodeState('fake_node%s' % x)
                      for x in range(1, 4)]
        ...
        with mock.patch.object(base_filter, 'LOG') as mock_log:
            result = self.handler.get_filtered_objects(
                all_filters, fake_nodes, filt_props)
            self.assertFalse(result)
            msg = "with volume ID '%s'" % fake.VOLUME_ID
            # FilterA should leave Node1 and Node2; FilterB should leave None.
            ...
            exp_output = ("[('FilterA', ['fake_node2', 'fake_node3']), "
                          "('FilterA', ['fake_node3']), "
                          + "('FilterB', None)]")
